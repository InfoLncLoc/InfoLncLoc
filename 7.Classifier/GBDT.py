...
def gbdt_modle(filename):
    feature = np.array(pd.read_csv('../8.SelfIndependTest/IndTrain/' + filename, header=None, low_memory=False, sep=',').iloc[1:, 1:])
    ...
    for train_index, test_index in kf.split(data, label):
        ...
        model = GradientBoostingClassifier(learning_rate=0.01, n_estimators=40, max_depth=4, min_samples_leaf=6, max_features=10,min_samples_split=2,  subsample=0.9, random_state=2019)
...
if __name__ == '__main__':
    ...
    asso_1['sp'] = sp_all
    asso_1['sn'] = sn_all
    asso_1.to_csv('./' + 'GBDTscore_train.csv', index=False)
